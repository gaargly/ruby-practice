...
find_first_nonzero_among(0, 0, 1, 0, 2, 0)
 ...
 # This means that the method expects one argument, not 6, as we only have one parameter listed in the method definition

find_first_nonzero_among(1)
# undefined method `each' for 1:Integer (NoMethodError)
# This means that each does not work for an integer. I know from previous experience that you need a data structure like an array for the .each method to work
...
# gets.chomp returns a string, so we need gets.chomp.to_i
...
# changed code (we were using map when we should've been using select)
...
# 5. You want to have a small script delivering motivational quotes, but with the following code you run into a very common error message: no implicit conversion of nil into String (TypeError). What is the problem and how can you fix it?
...
def get_quote(person)
  ...
  if person == 'Einstein'
    'Do not worry about your difficulties in Mathematics. I can assure you mine are still greater.' # this last value returns nil because Einstein is not passed into the method call, so the whole method returns nil
  end
end
...
def get_quote(person)
  ...
  return quote # we don't need to explicitly return quote but doing this here anyway to be super explicit
end

puts 'Confucius says:'
puts '"' + get_quote('Confucius') + '"'

# 6. Time for a check of your financial situation.
...
[january, february, march].each do |month|
  balance = calculate_balance(month) # this gets rewritten each time for each month so it's storing March's value in the end
end
...
def digit_product(str_num)
  ...
  product = 0 # multiplying any number by 0 always returns 0
...
# in Ruby documentation for the Array class, this is what Array#Merge does
